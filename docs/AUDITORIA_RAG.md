...
| RAG-05 | ALTO | Abierto | — | Recalibrar con el índice sano y el corpus real (después de RAG-02). Los logs no sirven: el RPC ya filtra por 0.65 y las similitudes por debajo nunca se registran. Medir con `POST /api/v1/admin/rag-probe` (requiere `x-admin-key`), que devuelve la similitud sin umbral de cada pregunta de prueba. |
