...
const fechaISOde = (d: Date) => d.toISOString().split('T')[0];

export const NewsSection = () => {
  const today = new Date();
  const fechaFormateada = formatearFecha(today);
  const fechaISO = fechaISOde(today);
  return (
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {news.map((item, index) => {
            ...
            itemDate.setDate(itemDate.getDate() - item.daysAgo);
            return (
            <article
              key={item.id}
            ...
            >
              ...
              <div className="p-5">
                <div className="flex items-center gap-2 text-muted-foreground text-sm mb-3">
                  <Calendar className="w-4 h-4" />
                  <time dateTime={fechaISOde(itemDate)}>{formatearFecha(itemDate)}</time>
                </div>
